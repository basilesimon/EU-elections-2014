...
module.exports = new function() {
     ...
     *
     */
    this.getCandidateById = function(candidateId) {
        var deferred = Q.defer();
        db.candidates.find({ '$query': { _id: candidateId }, '$orderby': { name: 1 } }, function(err, candidate) {
            deferred.resolve(candidate[0]);
        });
        return deferred.promise;
    }
    ...
    this.getRegions = function(country) {
        ...
        if (country)
            query.country = country;
        db.regions.find({ '$query': query, '$orderby': { name: 1 } }, function(err, regions) {
     ...
     *
     */
    this.getRegion = function(country, region) {
        var deferred = Q.defer();
        var query = {};
        if (country)
            query.country = country;
            if (country)
                query.name = region;
        db.regions.find({ '$query': query, '$orderby': { name: 1 } }, function(err, region) {
            deferred.resolve(region[0]);
        });
        return deferred.promise;
    }
